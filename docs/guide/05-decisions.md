...
- 현재 적용 요약과 작업 절차는 관련 `docs/guide/` 문서에서 확인한다.
- 상세 요구사항과 기술 명세는 관련 `docs/source/` 원문에서 확인한다.
...
- `docs/guide/`는 탐색 경로, 현재 적용 요약, 변경 절차를 관리한다.
- `docs/source/`는 요구사항, 아키텍처, API, DB 등 상세 판단의 원문을 관리한다.
- 동일한 상세 규칙을 guide와 source에 중복해서 관리하지 않는다.
- MVP는 `RUN` 중심이다.
